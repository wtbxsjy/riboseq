...
.melt_compat <- function(data, ...) {
    if (inherits(data, "table")) {
        ...
        tidyr::pivot_longer(data, cols = tidyr::everything(), 
...
setMethod(f = "unlist", "GRanges", function(x){return(x)})
...
get_orfs<-function(tx_name,sequence,get_all_starts=T,Stop_Stop=F,scores=c(1,.5),genetic_code_table){
    ...
    for(u in 0:2){
        ...
        if(length(st_st[,1]) == 0 || length(st_st[,2]) == 0 || any(is.na(st_st[,1])) || any(is.na(st_st[,2]))){
            list_frames[[paste("frame",u,sep = "_")]]<-GRanges(); next
        }
        if(get_all_starts==T){
