...
# PATCHED: Changed setMethods to setMethod (singular form) for R 4.x compatibility
setMethod(f = "unlist", "GRanges", function(x) {
    return(x)
})


DEFAULT_CIRC_SEQS <- unique(c(
    "chrM",
    "MT",
    "MtDNA",
    "mit",
...
# =============================================================================
# PERFORMANCE OPTIMIZATION v1.3: DPSS (Slepian) Caching
# =============================================================================
# The dpss() function is computationally expensive. Since many ORFs have similar
# lengths, we cache computed slepian values to avoid redundant calculations.

.dpss_cache <- new.env(hash = TRUE, parent = emptyenv())

#' Get cached DPSS (Slepian) values
#'
#' This function returns cached DPSS values if available, otherwise computes
#' and caches them. This significantly speeds up calc_orf_pval when many ORFs
#' have similar lengths.
#'
#' @param n Length parameter for dpss
#' @param k Number of tapers
#' @param nw Time-bandwidth parameter
#' @return DPSS object
#' @keywords internal
...
#' Clear DPSS cache
#'
#' Utility function to clear the DPSS cache when memory needs to be freed
#' @export
clear_dpss_cache <- function() {
...
get_orfs <- function(
    ...
    for (u in 0:2) {
        ...
        # PATCHED: Additional protection for IRanges - ensure vectors are valid
        if (
            length(st_st[, 1]) == 0 ||
                length(st_st[, 2]) == 0 ||
                any(is.na(st_st[, 1])) ||
        ...
        if (Stop_Stop == TRUE) {
            ...
            # Protection: check if st_st has valid rows before creating IRanges
            if (
                nrow(st_st) > 0 &&
                    length(st_st[, 1]) > 0 &&
                    length(st_st[, 2]) > 0
...
take_Fvals_spect <- function(x, n_tapers, time_bw, slepians_values) {
    if (length(x) < 25) {
        remain <- 50 - length(x)
        x <- c(
            rep(0, as.integer(remain / 2)),
            x,
            rep(0, remain %% 2 + as.integer(remain / 2))
        )
    }
    if (length(x) < 1024 / 2) {
        padding <- 1024
    ...
    freq_max_3nt <- resSpec1$freq[which(
        abs((resSpec1$freq - (1 / 3))) == min(abs((resSpec1$freq - (1 / 3))))
    )]

    Fmax_3nt <- resSpec1$mtm$Ftest[which(
        abs((resSpec1$freq - (1 / 3))) == min(abs((resSpec1$freq - (1 / 3))))
    )]
    spect_3nt <- resSpec1$spec[which(
        abs((resSpec1$freq - (1 / 3))) == min(abs((resSpec1$freq - (1 / 3))))
    )]
...
detect_translated_orfs <- function(
    selected_txs,
    genome_sequence,
    annotation,
    P_sites,
    ...
    for (tx in txs_sels) {
        ...
        orfs <- select_start(
            ORFs = orfs,
            P_sites_rle = covtx,
            cutoff = start_sel_cutoff,
            cutoff_ave = start_sel_cutoff_ave
        )
        if (length(orfs) == 0) {
            next
        }
        orfs <- calc_orf_pval(
            ORFs = orfs,
            P_sites_rle = covtx,
            P_sites_uniq_rle = covtx_uniq,
            P_sites_uniq_mm_rle = covtx_uniq_mm,
        ...
        tr_gen_tx <- tr_gen[
            as.vector(match(seqnames(orfs)[1], tr_gen[, "transcript_id"])),
        ]
        ...
        orfs$transcript_biotype <- unique(as.character(tr_gen_tx[,
            "transcript_biotype"
        ]))
    ...
    for (j in names(orfs_gr)) {
        ...
        if (length(featjuns) > 0) {
            over_tx <- sort(c(
                over_tx,
                featjuns[which(featjuns %in% gaps(orf_gen))]
            ))
        }
...
select_txs <- function(
    ...
    hts <- cbind(
        ...
        P_sites_uniq[subjectHits(hts)]$score *
            width(P_sites_uniq[subjectHits(hts)])
    )
    ...
    for (i in seq_len(ncol(mat))) {
        ...
        for (j in (seq_len(ncol(mat)))[-i]) {
            ...
            if (identical(yes, yesj)) {
                ident <- c(
                    ident,
                    paste(sort(colnames(mat)[c(i, j)]), collapse = ";")
                )
            }
            #added if length> otherwise txs with same structure are both deleted
            nesti <- c(
                nesti,
                sum(yesj %in% yes) == length(yes) &
                    length(mat[, j]) > length(yes)
            )
        }

        if (sum(nesti) > 0) {
            nest <- c(nest, colnames(mat)[i])
        }
    }
    if (length(ident) > 0) {
        ...
        for (i in seq_len(ncol(mat))) {
            ...
            if (sum(!tx_good %in% expl_good_old) > 0) {
                ...
                if (length(tx_toscreen) > 0) {
                    for (j in tx_toscreen) {
                        ...
                        if (
                            sum(tx_contr_good %in% tx_good) ==
                                length(tx_contr_good)
                        ) {
            ...
            if (sum(!tx_good %in% expl_good_old) == 0) {
                ...
                for (j in tx_toscreen) {
                    ...
                    if (
                        sum(tx_contr_good %in% tx_good) == length(tx_contr_good)
                    ) {
                        ...
                        if (length(tx_good) == length(tx_contr_good)) {
                            ...
                            contr_la <- which(
                                tx_contr == tx_contr_good[length(tx_contr_good)]
                            )
                            contr_int_tx <- tx_contr[contr_fi:contr_la]
                            contr_int_tx_bad <- tx_contr_bad[
                                tx_contr_bad %in% contr_int_tx
                            ]
                            #SAME INTERNAL, TAKE
                            if (
                                length(int_tx_bad) <= length(contr_int_tx_bad)
                            ) {
                                txs_good <- unique(c(
                                    txs_good,
                                    colnames(mat)[i]
    ...
    genes_sofar <- unique(
        subset(
            annotation$trann,
            annotation$trann$transcript_id %in% txs_sofar
        )$gene_id
    )
...
get_reathr_seq <- function(tx_name, orf, sequence, genetic_code) {
    ...
    #st_st<-st_st[(st_st[,2]-st_st[,1]>11),]
    # Protection: check if st_st has valid rows before creating IRanges
    if (nrow(st_st) > 0 && length(st_st[, 1]) > 0 && length(st_st[, 2]) > 0) {
        gra_par <- GRanges(
            seqnames = paste(tx_name, sep = "_"),
            strand = "+",
            ranges = IRanges(start = st_st[, 1], end = st_st[, 2])
        )
...
detect_readthrough <- function(
    ...
    P_sites_uniq <- P_sites_uniq[
        !P_sites_uniq %over% unlist(results_orf$ORFs_genomic_position)
    ]
    P_sites_uniq_mm <- P_sites_uniq_mm[
        !P_sites_uniq_mm %over% unlist(results_orf$ORFs_genomic_position)
    ]
    ...
    if (length(P_sites) > 4) {
        for (i in seq_along(results_orf$ORFs_tx_position)) {
            ...
            if (as.character(strand(ex_tx)[1]) == "+") {
                covtx <- suppressWarnings(unlist(coverage(
                    P_sites,
                    weight = P_sites$score
                )[ex_tx]))
                if (length(P_sites_uniq) > 0) {
                    covtx_uniq <- suppressWarnings(unlist(coverage(
                        P_sites_uniq,
                        weight = P_sites_uniq$score
                    )[ex_tx]))
                }
                if (length(P_sites_uniq) == 0) {
                    covtx_uniq <- suppressWarnings(unlist(coverage(
                ...
                if (length(P_sites_uniq_mm) > 0) {
                    covtx_uniq_mm <- suppressWarnings(unlist(coverage(
                        P_sites_uniq_mm,
                        weight = P_sites_uniq_mm$score
                    )[ex_tx]))
                }
                if (length(P_sites_uniq_mm) == 0) {
                    covtx_uniq_mm <- suppressWarnings(unlist(coverage(
            ...
            if (as.character(strand(ex_tx)[1]) == "-") {
                covtx <- suppressWarnings(unlist(unlist(RleList(lapply(
                    coverage(P_sites, weight = P_sites$score)[ex_tx],
                    FUN = rev
                )))))
                if (length(P_sites_uniq) > 0) {
                    covtx_uniq <- suppressWarnings(unlist(unlist(RleList(lapply(
                        coverage(P_sites_uniq, weight = P_sites_uniq$score)[
                            ex_tx
                        ],
                        FUN = rev
                    )))))
                ...
                if (length(P_sites_uniq) == 0) {
                    covtx_uniq <- suppressWarnings(unlist(unlist(RleList(lapply(
                        coverage(P_sites_uniq)[ex_tx],
                        FUN = rev
                    )))))
                }
                if (length(P_sites_uniq_mm) > 0) {
                    covtx_uniq_mm <- suppressWarnings(unlist(unlist(RleList(lapply(
                        coverage(
                            P_sites_uniq_mm,
                            weight = P_sites_uniq_mm$score
                        )[ex_tx],
                ...
                if (length(P_sites_uniq_mm) == 0) {
                    covtx_uniq_mm <- suppressWarnings(unlist(unlist(RleList(lapply(
                        coverage(P_sites_uniq_mm)[ex_tx],
                        FUN = rev
                    )))))
                }
            }
            ...
            orfs <- get_reathr_seq(
                ...
                orf = orf_tx,
                sequence = seq_tx,
                genetic_code = genetic_code_table
            )
            ...
            #uniq_flag
            if (!uniq_signal) {
                if (is.na(vals1$pval) | vals1$pct_fr < .5 | vals1$pval > .05) {
                    next
                }
            }
            if (uniq_signal) {
                if (
                    is.na(vals1$pval_uniq) |
                        vals1$pct_fr < .5 |
                        vals1$pval_uniq > .05
                ) {
                    next
            ...
            for (j in seq_along(readthroughs)) {
                ...
                readthroughs$gen_coords[j] <- from_tx_togen(
                    readthroughs[j],
                    annotation$exons_txs[as.character(seqnames(readthroughs[
                        j
                    ]))],
                    annotation$introns_txs[[as.character(seqnames(readthroughs[
                        j
                    ]))]]
                )
                readthroughs$compatible_id[j] <- CharacterList(mcs_orfs[which(
                    mcs$Protein == mcs[j, "Protein"]
                )[-j]])
                readthroughs$compatible_original_id[
                    j
                ] <- CharacterList(mcs_orfs_orig[which(
                    mcs$Protein == mcs[j, "Protein"]
                )[-j]])
            }
            dups <- duplicated(readthroughs$Protein) &
                duplicated(readthroughs$P_sites_raw)
...
select_quantify_ORFs <- function(
    ...
    if (length(select_feats_jun) > 0) {
        ...
        orfs_printWRONG <- apply(tab_j, MARGIN = 1, FUN = function(x) {
            colnames(tab_j)[which(x > 0)]
        })
    ...
    df_orfs_ex$exon_rank <- c(unlist(sapply(
        as.numeric(table(df_orfs_ex$tx_id)),
        FUN = function(x) {
            1:x
        }
    )))
    df_orfs <- data.frame(
        tx_id = sort(unique(df_orfs_ex$tx_id)),
        tx_name = as.character(names(orfs)),
    ...
    if (length(select_feats_jun) > 0) {
        ...
        if (!is(orfs_jj, "CompressedList")) {
            if (is.list(orfs_jj)) {
                gene_feat$tx_name <- CharacterList(c(
                    tx_ex,
                    CharacterList(orfs_jj)
                ))
            }
            if (!is.list(orfs_jj)) {
                gene_feat$tx_name <- CharacterList(c(
                    tx_ex,
                    CharacterList(as.list(orfs_jj))
                ))
            }
        }
    ...
    #change!
    gene_feat <- gene_feat_cp[
        !duplicated(paste(GRanges(gene_feat_cp), gene_feat_cp$type, sep = "_"))
    ]
    ...
    selected_ORFs <- lapply(results_ORFs, FUN = function(x) {
        x[names(x) %in% txs_good]
    })
    ...
    while (length(orf_del) > 0) {
        ...
        feats$ORF_id_tr_selected <- CharacterList(lapply(
            feats$ORF_id_tr_selected,
            function(x) {
                unique(x[!x %in% orf_del])
            }
        ))

        gene_feat$ORF_id_tr_selected_quant <- CharacterList(lapply(
            gene_feat$ORF_id_tr_selected_quant,
            function(x) {
                unique(x[!x %in% orf_del])
            }
        ))
        ...
        if (length(unqs_okk) == 0) {
            ...
            for (i in unqs_na) {
                ...
                for (j in seq_along(feat)) {
                    ...
                    if (sum(nass) > 1) {
                        adj <- (cov_feat[j] -
                            (cov_feat[j] * sum(unqs_adj[txs_fea]))) /
                            sum(nass)
                    }
                    if (length(txs_fea) == 0) {
        ...
        if (sum(unqs_okk == 0) == length(unqs_adj)) {
            ...
            for (i in unqs_zero) {
                ...
                for (j in seq_along(feat)) {
                    ...
                    if (sum(nass) > 1) {
                        adj <- (cov_feat[j] -
                            (cov_feat[j] * sum(unqs_okk[txs_fea]))) /
                            sum(nass)
                    }
                    if (length(txs_fea) == 0) {
                        adj <- cov_feat[j] / (sum(nass))
                ...
                if (length(unqs_okk) > 0) {
                    for (i in names(unqnas)) {
                        ...
                        for (j in seq_along(feat)) {
                            ...
                            txs_fea_adj <- txs_fea[
                                !txs_fea %in% names(unqs_okk_noi)
                            ]
                            adj <- cov_feat[j]
                            usef <- "shared"
                            if (length(txs_fea_adj) == 1) {
                                usef <- "unique"
                            }

                            if (
                                length(unqs_okk_noi_feat[
                                    !is.na(unqs_okk_noi_feat)
                                ]) >
                                    0
                            ) {
                                adj <- adj -
                                    (adj *
                                        sum(unqs_okk_noi_feat[
                                            !is.na(unqs_okk_noi_feat)
                                        ]))
                                if (adj < 0) {
                                    adj = 0
                        ...
                        if (sum(unq) > 0) {
                            unq_rat <- mean(adj_cov_feat[adj_use == "unique"]) /
                                mean(adj_cov_feat)
                            if (mean(adj_cov_feat) == 0) {
                                unq_rat <- 0
                            }
                            if (unq_rat > 1) {
                                unq_rat <- 1
                            ...
                            orfs_tx[[
                                i
                            ]]$adj_unique_features_reads <- NumericList(riz[
                                unq
                            ])
                        }
                        if (sum(unq) == 0) {
                            orfs_tx[[
                                i
                            ]]$adj_unique_features_reads <- NumericList(NA)
                            unq_rat <- NA
                        }
                        unqs_adj[i] <- unq_rat
                    }
        ...
        if (!is(feats, "GRangesList")) {
            feats <- GRangesList(feats)
        }
        featsall <- sort(unlist(feats))
        featsall <- featsall[
            !duplicated(paste(GRanges(featsall), featsall$type, sep = "_"))
        ]
        ...
        if (optimiz == TRUE) {
            ...
            expect <- unlist(lapply(featsall$ORF_id_tr_selected, function(x) {
                sum(vals[x])
            }))
            ...
            calc_dist_exp <- function(unqs_optim, maxval = maxdist) {
                ...
                expect <- unlist(lapply(
                    featsall$ORF_id_tr_selected,
                    function(x) {
                        sum(vals[x])
                    }
                ))
            ...
            optimm <- optim(
                par = unqs_optim,
                fn = calc_dist_exp,
                lower = rep(0, length(unqs_optim)),
                upper = rep(1, length(unqs_optim)),
                method = "L-BFGS-B"
            )
            unqs_optim <- optimm$par
            if (sum(unqs_optim > 0) == 0) {
                unqs_optim <- unqs_optim_noopt
            }
        }
        ...
        for (i in names(feats)) {
            ...
            orfs_tx[[i]]$scaling_factors <- NumericList(round(
                scalss,
                digits = 4
            ))
        }
        ...
        orfs_genes <- split(
            unlist(GRangesList(orfs_tx)),
            f = unlist(GRangesList(orfs_tx))$gene_id
        )
        orfs_genes <- GRangesList(lapply(orfs_genes, FUN = function(x) {
            ...
            x$ORF_pct_P_sites_pN <- (x$P_sites / width(x)) *
                100 /
                sum(x$P_sites / width(x))
        ...
        if (is.numeric(cutoff_pct)) {
            ...
            orf_del_iso <- unique(c(
                orf_del_iso,
                orfgrlun$ORF_id_tr[which(orfgrlun$ORF_pct_P_sites < cutoff_pct)]
            ))
        }

        if (is.numeric(cutoff_P_sites)) {
            orfgrlun <- unlist(orfs_genes)
            orf_del_ps <- orfgrlun$ORF_id_tr[which(
                orfgrlun$P_sites < cutoff_P_sites
            )]
        }
        ...
        for (g in names(orfs_tx)) {
            orfs_tx[[g]]$ORF_pct_P_sites <- round(
                fs$ORF_pct_P_sites[fs$ORF_id_tr == g],
                digits = 4
            )
            orfs_tx[[g]]$ORF_pct_P_sites_pN <- round(
                fs$ORF_pct_P_sites_pN[fs$ORF_id_tr == g],
                digits = 4
            )
        }
        counter <- counter + 1
    ...
    results_ORFs$ORFs_features <- results_ORFs$ORFs_features[names(
        results_ORFs$ORFs_tx_position
    )]
    results_ORFs$tx_annotated_ORFs <- results_ORFs$tx_annotated_ORFs[names(
        results_ORFs$ORFs_tx_position
    )]
    #adjust feats!
...
annotate_splicing <- function(orf_gen, ref_cds) {
    ...
    if (runValue(rlesp)[1] == "new_miss") {
        if (as.character(strand(spl_ran)[1]) == "+") {
            spl_ran$spl_type[1:runLength(rlesp)[1]] <- paste(
                spl_ran$spl_type[1:runLength(rlesp)[1]],
                "_5prime",
                sep = ""
            )
        }
    ...
    if (runValue(rlesp)[lenna] == "new_miss") {
        if (as.character(strand(spl_ran)[1]) == "+") {
            spl_ran$spl_type[
                (length(spl_ran) - (runLength(rlesp)[lenna] - 1)):length(
                    spl_ran
                )
            ] <- paste(
                spl_ran$spl_type[
    ...
    spl_ran$spl_type <- gsub(
        spl_ran$spl_type,
        pattern = "_5prime_3prime",
        replacement = "_notoverl"
    )
    spl_ran
}
...
annotate_ORFs <- function(
    ...
    annotated_cds_tx_genes <- Annotation$trann$gene_id[match(
        names(annotated_cds_tx),
        Annotation$trann$transcript_id
    )]
    annotated_exons_tx <- Annotation$exons_txs[
        Annotation$exons_txs %over% region
    ]
    ...
    for (i in names(ORFs_gen)) {
        ...
        comp <- sapply(redmapp, function(x) {
            paste(seqnames(x), start(x), end(x), sep = "_")
        })
        names(comp) <- NULL
        ...
        ORFs_tx[[i]]$compatible_biotype_longest <- ORFs_tx[[
            i
        ]]$compatible_biotype
        ORFs_tx[[i]]$compatible_tx_longest <- ORFs_tx[[i]]$compatible_tx
        ORFs_tx[[i]]$compatible_ORF_id_tr_longest <- ORFs_tx[[
            i
        ]]$compatible_ORF_id_tr
        lng <- ORFs_tx[[i]]$longest_ORF
        lng$ORF_id_tr <- paste(seqnames(lng), start(lng), end(lng), sep = "_")
        if (start(ORFs_tx[[i]]) != start(lng)) {
            x <- from_tx_togen(
                ORFs = lng,
                exons = Annotation$exons_txs[ORFs_tx[[i]]$transcript_id],
                introns = Annotation$introns_txs[[ORFs_tx[[i]]$transcript_id]]
            )[[1]]
            mapp <- mapToTranscripts(x, transcripts = annotated_exons_tx)
            redmapp <- reduce(split(mapp, seqnames(mapp)))
            ...
            ORFs_tx[[i]]$compatible_ORF_id_tr_longest <- paste(
                as.character(seqnames(lng)[1]),
                start(lng),
                end(lng),
                sep = "_"
            )
            ...
            if (length(comp_txs) > 0) {
                compid <- sapply(comp_txs, function(x) {
                    txs <- sapply(strsplit(x, split = "_"), function(x) {
                        paste(x[-((length(x) - 1):length(x))], collapse = "_")
                    })
                    btps <- Annotation$trann$transcript_biotype[match(
                        txs,
                        Annotation$trann$transcript_id
                ...
                ORFs_tx[[i]]$compatible_ORF_id_tr_longest[compats] <- t(
                    compid
                )[, 1]
                ORFs_tx[[i]]$compatible_tx_longest[compats] <- t(compid)[, 2]
                ORFs_tx[[i]]$compatible_biotype_longest[compats] <- t(compid)[,
                    3
                ]
            }
        }
    ...
    last_exexs <- start(txs_all)[match(
        unlist(GRangesList(ORFs_tx))$transcript_id,
        names(txs_all)
    )]
    ...
    for (i in seq_along(ORFs_tx)) {
        ...
        if (length(annotated_ORF) > 0) {
            ...
            if (sto == ann_sto) {
                if (sta == ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "ORF_annotated"
                }
                if (sta < ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "N_extension"
                }
                if (sta > ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "N_truncation"
                }
            }
            if (sto != ann_sto) {
                if (sta < ann_sta & sto < ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "overl_uORF"
                }
                if (sta < ann_sta & sto < ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "uORF"
                }
                if (sta < ann_sta & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "NC_extension"
                }
                if (sta > ann_sta & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "overl_dORF"
                }
                if (sta > ann_sto & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "dORF"
                }
                if (sta > ann_sta & sto < ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "nested_ORF"
                }
                if (sta == ann_sta & sto < ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "C_truncation"
                }
                if (sta == ann_sta & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx <- "C_extension"
                }
            }
        }
        ...
        if (length(annotated_ORF_compatible) > 0) {
            ...
            #change
            sta <- as.numeric(sapply(
                strsplit(orf_tx$compatible_ORF_id_tr, "_"),
                function(x) {
                    x[length(x) - 1]
                }
            ))
            sto <- as.numeric(sapply(
                strsplit(orf_tx$compatible_ORF_id_tr, "_"),
                function(x) {
                    x[length(x)]
                }
            ...
            if (sto == ann_sto) {
                if (sta == ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "ORF_annotated"
                }
                if (sta < ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "N_extension"
                }
                if (sta > ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "N_truncation"
                }
            }
            if (sto != ann_sto) {
                if (sta < ann_sta & sto < ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "overl_uORF"
                }
                if (sta < ann_sta & sto < ann_sta) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "uORF"
                }
                if (sta < ann_sta & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "NC_extension"
                }
                if (sta > ann_sta & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "overl_dORF"
                }
                if (sta > ann_sto & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "dORF"
                }
                if (sta > ann_sta & sto < ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "nested_ORF"
                }
                if (sta == ann_sta & sto < ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "C_truncation"
                }
                if (sta == ann_sta & sto > ann_sto) {
                    ORFs_tx[[i]]$ORF_category_Tx_compatible <- "C_extension"
                }
            }
        }
        ...
        if (length(annotated_cds) == 1) {
            annotated_cds2 <- reduce(unlist(annotated_cds))
        }
        if (length(annotated_cds) == 0) {
            annotated_cds2 <- annotated_cds
        }
        ...
        if (sum(overl) > 0) {
            ...
            if (length(max_cdsok) == 0) {
                max_cdsok <- annotated_cds_tx[[which.max(sum(width(
                    annotated_cds_tx
                )))]]
            }
            ...
            if (length(max_cds_seq) < 33 | nchar(orf_tx$Protein) < 10) {
                segm <- min(c(
                    as.integer(length(max_cds_seq) / 3) - 1,
                    nchar(orf_tx$Protein)
                ))
            }
            N_pr <- AAString(unlist(orf_tx$Protein))[1:segm]
            C_pr <- AAString(unlist(orf_tx$Protein))[
                (nchar(unlist(orf_tx$Protein)) - (segm - 1)):nchar(unlist(
                    orf_tx$Protein
                ))
            ]
            N_ann <- translate(
                max_cds_seq[1:(segm * 3)],
                genetic.code = genetic_code,
                if.fuzzy.codon = "solve"
            )
            C_ann <- translate(
                head(tail(max_cds_seq, (segm * 3 + 3)), (segm * 3)),
                genetic.code = genetic_code,
                if.fuzzy.codon = "solve"
            )
            ...
            if (N_pr == N_ann) {
                if (C_pr == C_ann) {
                    ...
                    ORFs_tx[[i]]$NC_protein_isoform <- "C"
                }
            }
            ...
            if (as.vector(strand(orf_gen[1])) == "+") {
                ...
                if (sto_or == gen_sto) {
                    if (sta_or == gen_sta) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "exact_start_stop"
                    }
                    if (sta_or < gen_sta) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt5_start"
                    }
                    if (sta_or > gen_sta) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt3_start"
                    }
                }

                if (sto_or != gen_sto) {
                    if (sta_or < gen_sta & sto_or < gen_sto) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt5_start_Alt5_stop"
                    }
                    if (sta_or < gen_sta & sto_or > gen_sto) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt5_start_Alt3_stop"
                    }
                    if (sta_or > gen_sta & sto_or > gen_sto) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt3_start_Alt3_stop"
                    }
                    if (sta_or > gen_sta & sto_or < gen_sto) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt3_start_Alt5_stop"
                    }
                    if (sta_or == gen_sta & sto_or < gen_sto) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt5_stop"
                    }
                    if (sta_or == gen_sta & sto_or > gen_sto) {
                        ORFs_tx[[i]]$ORF_category_Gen <- "Alt3_stop"
                    }
                }
            }
            if (as.vector(strand(orf_gen[1])) == "-") {
        ...
        ORFs_splice_feats[[orf_tx$ORF_id_tr]] <- annotate_splicing(
            orf_gen = orf_gen,
            ref_cds = max_cdsok
        )
        #to maxORF
...
ORFquant <- function(
    ...
    P_sites_uniq_region <- for_ORFquant$P_sites_uniq[
        for_ORFquant$P_sites_uniq %over% region
    ]
    P_sites_uniq_mm_region <- for_ORFquant$P_sites_uniq_mm[
        for_ORFquant$P_sites_uniq_mm %over% region
    ]

    res_orfs <- list()
    minimum_reads <- length(P_sites_region) > 4
    if (unique_reads) {
        length(P_sites_uniq_region) > 4
    }

    if (minimum_reads) {
        selected_transcripts <- select_txs(
            region = region,
        ...
        if (length(res_orfs) > 0) {
            res_orfs <- select_quantify_ORFs(
                results_ORFs = res_orfs,
                P_sites = P_sites_region,
                P_sites_uniq = P_sites_uniq_region,
                cutoff_cums = orf_quant.cutoff_cums,
                cutoff_pct = orf_quant.cutoff_pct,
                cutoff_P_sites = orf_quant.cutoff_P_sites,
                uniq_signal = unique_reads
            )
        }
        if (length(res_orfs) > 0) {
            ...
            res_orfs[["readthrough"]] <- suppressWarnings(detect_readthrough(
                results_orf = res_orfs,
                P_sites = P_sites_region,
                P_sites_uniq = P_sites_uniq_region,
                P_sites_uniq_mm = P_sites_uniq_mm_region,
                genome_sequence = genome_sequence,
...
run_ORFquant <- function(
    for_ORFquant_file,
    annotation_file,
    n_cores,
    prefix = for_ORFquant_file,
    gene_name = NA,
    gene_id = NA,
    genomic_region = NA,
    ...
    parallel_backend = c("auto", "snow", "serial", "fork", "mirai")
) {
    ...
    if (parallel_backend == "mirai" && !requireNamespace("mirai", quietly = TRUE)) {
        stop("parallel_backend = 'mirai' requires the mirai package. Install with: install.packages('mirai')")
    }
    ...
    cat(paste(
        "Loading annotation and Ribo-seq signal ... ",
        date(),
        "\n",
        sep = ""
    ))
    ...
    if (length(for_ORFquant_file) > 1) {
                    ...
                    tryCatch(
                        {
                            !is.null(gr) &&
                                length(gr) > 0 &&
                                all(is.finite(start(gr))) &&
                                all(is.finite(end(gr))) &&
                                all(width(gr) > 0)
                        },
        ...
        if (!any(haspsites)) {
            stop('None of the supplied objects have p sites')
        }

        message('loading')
        message(nonzeroloadobs[[1]])
        for_ORFquant_data <- load_obj(nonzeroloadobs[1])
        for (i in seq_along(nonzeroloadobs)[-1]) {
            # for(i in 2){
            message('loading')
            message(nonzeroloadobs[[i]])
            for_ORFquant_data2 <- load_obj(nonzeroloadobs[[i]])
            for_ORFquant_data$P_sites_all <- add_scoregranges(list(
                for_ORFquant_data$P_sites_all,
                for_ORFquant_data2$P_sites_all
            ))
            for_ORFquant_data$P_sites_uniq <- add_scoregranges(list(
                for_ORFquant_data$P_sites_uniq,
                for_ORFquant_data2$P_sites_uniq
            ))
            for_ORFquant_data$P_sites_uniq_mm <- add_scoregranges(list(
                for_ORFquant_data$P_sites_uniq_mm,
                for_ORFquant_data2$P_sites_uniq_mm
            ))
    ...
    if (!is.null(for_ORFquant_data$P_sites_all)) {
        ...
        if (is(for_ORFquant_data$P_sites_all, "GRangesList")) {
            ...
            for_ORFquant_data$P_sites_all <- for_ORFquant_data$P_sites_all[
                valid_idx
            ]
            ...
            if (length(for_ORFquant_data$P_sites_all) == 0) {
                warning(
                    "All P-site data elements were invalid or empty. Creating empty GRangesList."
                )
            ...
            tryCatch(
                ...
                error = function(e) {
                    warning(
                        "Error filtering P-site GRanges: ",
                        conditionMessage(e)
                    )
    ...
    if (!is.na(gene_name[1])) {
        gnid <- unique(GTF_annotation$trann$gene_id[
            GTF_annotation$trann$gene_name %in% gene_name
        ])
    ...
    if (length(genes_red) == 0) {
        stop(paste(
            "Incorrect gene_id | gene_name | genomic_region! ",
            date(),
            sep = ""
        ))
    }
    ...
    cat(paste(
        "Loading annotation and Ribo-seq signal --- Done! ",
        date(),
        "\n",
        sep = ""
    ))

    ovs_genesred <- summarizeOverlaps(
        genes_red,
        reads = for_ORFquant_data$P_sites_all
    )
    ...
    if (length(genes_red) == 0) {
        stop(paste(
            "Not enough P_sites signal over genomic regions! ",
            date(),
            sep = ""
        ))
    }
    ...
    cat(paste(
        "Summoning ORFquant with ",
        sum(for_ORFquant_data$P_sites_all %over% genes_red),
        " P_sites positions over ",
        length(genes_red),
        ...
        tryCatch(
            {
                ...
                if (canonical_start_only) {
                    attributes(genetcd)$alt_init_codons <- names(which(
                        genetcd == "M"
                    ))
                }
    ...
    if (write_temp_files) {
        save(
            ORFs_found,
            file = paste(prefix, "tmp_ORFquant_results", sep = "_")
        )
    }
    ...
    ORFquant_results <- list(
        ...
        ORFs_gen,
        ORFs_feat,
        ORFs_spl_feat_longest,
        ORFs_spl_feat_maxORF,
        ORFs_readthroughs,
        ORFs_txs_feats,
        selected_txs
    )
    names(ORFquant_results) <- c(
        "ORFs_tx",
        "ORFs_gen",
        "ORFs_feat",
        "ORFs_spl_feat_longest",
    ...
    save(
        ORFquant_results,
        file = paste(prefix, "final_ORFquant_results", sep = "_")
    )
    ...
    if (write_GTF_file) {
        ...
        ORFs_gen$transcript_biotype <- map_tx_genes[
            match_tx,
            "transcript_biotype"
        ]
        ...
        ORFs_gen$P_sites <- round(
            map_tx_genes[match_ORF, "P_sites"],
            digits = 4
        )
        ORFs_gen$ORF_pct_P_sites <- round(
            map_tx_genes[match_ORF, "ORF_pct_P_sites"],
            digits = 4
        )
        ORFs_gen$ORF_pct_P_sites_pN <- round(
            map_tx_genes[match_ORF, "ORF_pct_P_sites_pN"],
            digits = 4
        )
        ORFs_gen$ORFs_pM <- round(
            map_tx_genes[match_ORF, "ORFs_pM"],
            digits = 4
        )
        ...
        if (length(proteins_readthrough) > 0) {
            ...
            proteins_readthrough <- narrow(
                proteins_readthrough,
                start = start(proteins_readthrough)[1] + 1
            )
        ...
        if (write_protein_fasta) {
            writeXStringSet(
                proteins,
                filepath = paste(prefix, "Protein_sequences.fasta", sep = "_")
            )
        }
        ...
        exs_gtf$transcript_biotype <- map_tx_genes[
            match(exs_gtf$transcript_id, map_tx_genes$transcript_id),
            "transcript_biotype"
        ]
        exs_gtf$gene_id <- map_tx_genes[
            match(exs_gtf$transcript_id, map_tx_genes$transcript_id),
            "gene_id"
        ]
        exs_gtf$gene_biotype <- map_tx_genes[
            match(exs_gtf$transcript_id, map_tx_genes$transcript_id),
            "gene_biotype"
        ]
        exs_gtf$gene_name <- map_tx_genes[
            match(exs_gtf$transcript_id, map_tx_genes$transcript_id),
            "gene_name"
        ]
        mcols(exs_gtf)[, names(mcols(ORFs_gen))[
            !names(mcols(ORFs_gen)) %in% names(mcols(exs_gtf))
        ]] <- NA
        exs_gtf$type <- "exon"
...
load_annotation <- function(path, keep_fafile = FALSE) {
    ...
    if (inherits(genome_sequence, "FaFile")) {
        # Extract genome reference (path + circular ranges) for daemon re-opening
        ann$genome_ref <- .orfquant_genome_ref(genome_sequence)

        if (keep_fafile) {
            # mirai/snow backends: keep FaFile for disk-backed access (~0 MB RAM per daemon).
            # Daemons open independent FaFile handles from genome_ref$path.
            # Do NOT convert to DNAStringSet — the 2+ GB conversion is unnecessary
            # for socket-based parallelism.
            genome_sequence <- genome_sequence  # keep as FaFile
        } else {
            # fork/serial backends: convert to DNAStringSet for fork safety.
            # FaFile C++ external pointers cause finalizer crashes in fork
            # children (mclapply).  DNAStringSet is a pure-R object.
...
prepare_annotation_files <- function(
    ...
    if (!dir.exists(annotation_directory)) {
        dir.create(path = annotation_directory, recursive = TRUE)
    }
    ...
    if (forge_BSgenome) {
        ...
        if (!ok) {
            stop(
                "\"scientific_name\" must be two words separated by a \".\", like \"Homo.sapiens\""
            )
        }
        ...
        forgeBSgenomeDataPkg(
            x = seed_dest,
            destdir = annotation_directory,
            seqs_srcdir = dirname(twobit_file)
        )
        cat(paste(
            "Creating the BSgenome package --- Done! ",
            date(),
            "\n",
            sep = ""
        ))
        ...
        cat(paste(
            "Installing the BSgenome package ... ",
            date(),
            "\n",
            sep = ""
        ))
        ...
        cat(paste(
            "Installing the BSgenome package --- Done! ",
            date(),
            "\n",
            sep = ""
        ))
    ...
    if (create_TxDb) {
        ...
        if (
            sum(!is.na(trann$transcript_biotype)) == 0 &
                sum(!is.na(trann$transcript_type)) == 0
        ) {
        ...
        if (
            sum(!is.na(trann$gene_biotype)) == 0 &
                sum(!is.na(trann$gene_type)) == 0
        ) {
            trann$gene_type <- "no_type"
        }
        if (
            sum(!is.na(trann$gene_name)) == 0 &
                sum(!is.na(trann$gene_symbol)) == 0
        ) {
        ...
        g <- unlist(apply(
            cbind(seq_along(mat_genes), Y = elementNROWS(mat_genes)),
        ...
        ncrnas <- nc_exons[
            !nc_exons %over%
                genes[trann$gene_id[trann$gene_biotype == "protein_coding"]]
        ]
        ncisof <- nc_exons[
            nc_exons %over%
                genes[trann$gene_id[trann$gene_biotype == "protein_coding"]]
        ...
        #Mammalian mito
        translations$genetic_code[
            rownames(translations) %in%
                c("chrM", "MT", "MtDNA", "mit", "mitochondrion")
        ] <- "2"
        ...
        trsl <- suppressWarnings(translate(
            seqcds,
            genetic.code = getGeneticCode(cd),
            if.fuzzy.codon = "solve"
        ))
        trslend <- as.character(narrow(trsl, end = width(trsl), width = 1))
        stop_inannot <- NA
        if (names(sort(table(trslend), decreasing = TRUE)[1]) == "*") {
            stop_inannot <- "*"
        }

        cds_txscoords$gene_id <- trann$gene_id[match(
            as.vector(seqnames(cds_txscoords)),
            trann$transcript_id
        )]
        cds_cc <- cds_txscoords
        strand(cds_cc) <- "*"
        sta_cc <- resize(cds_cc, width = 1, "start")
        sta_cc <- unlist(pmapFromTranscripts(
            sta_cc,
            exons_tx[seqnames(sta_cc)],
            ignore.strand = FALSE
        ))
        sta_cc$gene_id <- trann$gene_id[match(
            names(sta_cc),
            trann$transcript_id
        )]
        sta_cc <- sta_cc[sta_cc$hit]
        strand(sta_cc) <- structure(
            as.vector(strand(transcripts_db)),
            names = transcripts_db$tx_name
        )[names(sta_cc)]
        ...
        sto_cc <- unlist(pmapFromTranscripts(
            sto_cc,
            exons_tx[seqnames(sto_cc)],
            ignore.strand = FALSE
        ))
        sto_cc <- sto_cc[sto_cc$hit]
        sto_cc$gene_id <- trann$gene_id[match(
            names(sto_cc),
            trann$transcript_id
        )]
        strand(sto_cc) <- structure(
            as.vector(strand(transcripts_db)),
            names = transcripts_db$tx_name
        )[names(sto_cc)]
        ...
        cat(paste(
            "Defining most common start/stop codons ... ",
            date(),
            "\n",
            sep = ""
        ))
        ...
        mostfr <- by(
            ...
            function(x) {
                mfreq <- table(x)
                x$start_pos %in%
                    as.numeric(names(which(mfreq[, 1] == max(mfreq[, 1])))) |
                    x$start_pos %in%
                        as.numeric(names(which(mfreq[, 2] == max(mfreq[, 2]))))
            }
        )
        ...
        mostupstr_tx <- sum(LogicalList(split(
            start_stop_cc$most_up_downstream,
            start_stop_cc$transcript_id
        )))[as.character(seqnames(cds_txscoords))]
        cds_txscoords$upstr_stasto <- mostupstr_tx
        mostfreq_tx <- sum(LogicalList(split(
            start_stop_cc$most_frequent,
            start_stop_cc$transcript_id
        )))[as.character(seqnames(cds_txscoords))]
        cds_txscoords$mostfreq_stasto <- mostfreq_tx
        cds_txscoords$lentx <- sum(width(exons_tx[as.character(seqnames(
            cds_txscoords
        ))]))
        df <- cbind.DataFrame(
            as.character(seqnames(cds_txscoords)),
        ...
        repres_len5 <- by(
            ...
            function(x) {
                ...
                ok <- x$txid[which(
                    x$utr5len == max(x$utr5len) & x$var == max(x$var)
                )][1]
                if (length(ok) == 0 | is.na(ok[1])) {
                    ok <- x$txid[1]
                }
        ...
        cds_txscoords$reprentative_mostcommon <- as.character(seqnames(
            cds_txscoords
        )) %in%
            unlist(repres_freq)
        cds_txscoords$reprentative_boundaries <- as.character(seqnames(
            cds_txscoords
        )) %in%
            unlist(repres_upstr)
        cds_txscoords$reprentative_5len <- as.character(seqnames(
            cds_txscoords
        )) %in%
            unlist(repres_len5)
        ...
        unq_stst$reprentative_mostcommon <- sum(
            !is.na(match(
                unq_stst$transcript_id,
                unlist(as(repres_freq, "CharacterList"))
            ))
        ) >
            0
        unq_stst$reprentative_boundaries <- sum(
            !is.na(match(
                unq_stst$transcript_id,
                unlist(as(repres_upstr, "CharacterList"))
            ))
        ) >
            0
        unq_stst$reprentative_5len <- sum(
            !is.na(match(
                unq_stst$transcript_id,
                unlist(as(repres_len5, "CharacterList"))
            ))
        ) >
            0
        ...
        if (export_bed_tables_TxDb == TRUE) {
            ...
            for (bed_file in c(
                ...
                for (mecc in names(meccole)) {
                    if (
                        is(meccole[, mecc], "CharacterList") |
                            is(meccole[, mecc], "NumericList") |
                            is(meccole[, mecc], "IntegerList")
                    ) {
                        meccole[, mecc] <- paste(
                            meccole[, mecc],
                            collapse = ";"
            ...
            write.table(
                seqi,
                file = paste(annotation_directory, "/seqinfo", sep = ""),
                sep = "\t",
                quote = FALSE,
                row.names = FALSE
            )
            ...
            write.table(
                gen_cod,
                file = paste(annotation_directory, "/genetic_codes", sep = ""),
                sep = "\t",
                quote = FALSE,
                row.names = FALSE
            )
            cat(paste(
                "Exporting annotation tables --- Done! ",
                date(),
                "\n",
                sep = ""
            ))
...
get_ps_fromspliceplus <- function(x, cutoff) {
    ...
    if (sum(ones) > 0) {
        psones <- shift(
            resize(gr[ones], width = 1, fix = "start"),
            shift = cutoff
        )
    }
    ...
    if (sum(mores) > 0) {
        ...
        psmores <- GRanges(
            ...
            seqnames = seqnames(x[mores]),
            strand = strand(x[mores]),
            seqlengths = seqlengths(x[mores])
        )
    }
...
prepare_for_ORFquant <- function(
    ...
    if (
        is.na(path_to_rl_cutoff_file) &
            is.na(path_to_P_sites_plus_bw) &
            is.na(path_to_P_sites_minus_bw)
    ) {
        stop(paste(
            "Please input either the paths to the P_sites bw files, or the path a suitable rl_cutoff table! ",
            date(),
            sep = ""
        ))
    }
    ...
    if (!is.na(path_to_rl_cutoff_file)) {
        ...
        for (compar in compnms) {
            cat(paste(
                "Using ",
                paste(rl_cutoffs_comp[[compar]][, 1], collapse = ","),
                " nt long footprints with ",
                paste(rl_cutoffs_comp[[compar]][, 2], collapse = ","),
                " as cutoffs, '",
                compar,
                "' compartment ... ",
                "\n",
    ...
    circs_seq <- seqnames(GTF_annotation$seqinfo)[which(isCircular(
    ...
    if (!is.na(path_to_P_sites_plus_bw)) {
        ...
        suppressWarnings(
            input_P_sites <- sort(c(input_P_sites_pl, input_P_sites_mn))
        )
    ...
    if (!is.na(path_to_P_sites_uniq_plus_bw)) {
        ...
        suppressWarnings(
            input_P_sites_uniq <- sort(c(
                input_P_sites_uniq_pl,
                input_P_sites_uniq_mn
            ))
        )
    ...
    if (!is.na(path_to_P_sites_uniq_mm_plus_bw)) {
        ...
        suppressWarnings(
            input_P_sites_uniq <- sort(c(
                input_P_sites_uniq_mm_pl,
                input_P_sites_uniq_mm_mn
            ))
        )
        seqlevels(input_P_sites_uniq, pruning.mode = "coarse") <- seqllll
        seqlengths(input_P_sites_uniq) <- seqleee
    ...
    reduc <- function(x, y) {
        ...
        if (!is.na(path_to_rl_cutoff_file)) {
            ...
            uniq_ps <- GRangesList()
            rls <- unique(c(
                names(x[["P_sites_uniq"]]),
                names(y[["P_sites_uniq"]])
            ))
            ...
            rls <- unique(c(
                names(x[["P_sites_uniq_mm"]]),
                names(y[["P_sites_uniq_mm"]])
            ))
        ...
        rang_jun$unique_reads <- rang_jun$unique_reads +
            y$junctions$unique_reads
        ...
        if (!is.na(path_to_rl_cutoff_file)) {
            ...
            for (comp in names(rl_cutoffs_comp)) {
                ...
                for (i in seq_along(resul$read_length)) {
                    ...
                    ok_reads <- ok_reads[
                        as.vector(seqnames(ok_reads)) %in% chroms
                    ]
                    ...
                    if (length(ok_reads) > 0) {
                        ...
                        ps_unspl <- shift(
                            resize(GRanges(unspl), width = 1, fix = "start"),
                            shift = ct
                        )
                        ...
                        firstb <- as.numeric(sapply(
                            strsplit(cigar(spl), "M"),
                            "[[",
                            1
                        ))
                        lastb <- as.numeric(sapply(
                            strsplit(cigar(spl), "M"),
                            function(x) {
                                gsub(
                                    x[length(x)],
                        ...
                        firstok <- shift(
                            resize(GRanges(firstok), width = 1, fix = "start"),
                            shift = ct
                        )

                        lastok <- spl[lastb >= rl - ct]
                        lastok <- shift(
                            resize(GRanges(lastok), width = 1, fix = "end"),
                            shift = -(rl - ct - 1)
                        )
                        ...
                        ps_plus_uniq_mm <- ps_plus[
                            mcols(ps_plus)$mapq > 50 &
                                nchar(mcols(ps_plus)$MD) > 3
                        ]
                    ...
                    if (length(ok_reads) > 0) {
                        ...
                        lastok <- spl[lastb > ct]
                        lastok <- shift(
                            resize(GRanges(lastok), width = 1, fix = "start"),
                            shift = -ct
                        )

                        firstok <- spl[firstb >= rl - ct]
                        firstok <- shift(
                            resize(GRanges(firstok), width = 1, fix = "end"),
                            shift = (rl - ct - 1)
                        )
                        ...
                        ps_neg_uniq_mm <- ps_neg[
                            mcols(ps_neg)$mapq > 50 &
                                nchar(mcols(ps_neg)$MD) > 3
                        ]
            ...
            seqlevels(all_ps_comps) <- seqllll
            seqlengths(all_ps_comps) <- seqleee
            rls_comps <- unique(unlist(lapply(list_pss, FUN = function(x) {
                names(x[["P_sites_all"]])
            })))
            for (rl in rls_comps) {
                ...
                for (comp in names(list_pss)) {
                    if (
                        sum(rl %in% names(list_pss[[comp]][["P_sites_all"]])) >
                            0
                    ) {
            ...
            for (rl in rls_comps) {
                ...
                for (comp in names(list_pss)) {
                    if (
                        sum(rl %in% names(list_pss[[comp]][["P_sites_uniq"]])) >
                            0
                    ) {
            ...
            for (rl in rls_comps) {
                ...
                for (comp in names(list_pss)) {
                    if (
                        sum(
                            rl %in% names(list_pss[[comp]][["P_sites_uniq_mm"]])
                        ) >
                            0
                    ) {
    ...
    if (length(for_ORFquant$P_sites_all) > 0) {
        ...
        if (length(merged_all_ps) > 0) {
            covv_pl <- coverage(
                merged_all_ps[strand(merged_all_ps) == "+"],
                weight = merged_all_ps[strand(merged_all_ps) == "+"]$score
            )
            covv_pl <- GRanges(covv_pl)
            covv_pl <- covv_pl[covv_pl$score > 0]
            covv_min <- coverage(
    ...
    if (length(for_ORFquant$P_sites_uniq) > 0) {
        ...
        if (length(merged_uniq_ps) > 0) {
            covv_pl <- coverage(
                merged_uniq_ps[strand(merged_uniq_ps) == "+"],
                weight = merged_uniq_ps[strand(merged_uniq_ps) == "+"]$score
            )
            covv_pl <- GRanges(covv_pl)
            covv_pl <- covv_pl[covv_pl$score > 0]
    ...
    if (length(for_ORFquant$P_sites_uniq_mm) > 0) {
        ...
        if (length(merged_uniq_mm_ps) > 0) {
            covv_pl <- coverage(
                merged_uniq_mm_ps[strand(merged_uniq_mm_ps) == "+"],
                weight = merged_uniq_mm_ps[
                    strand(merged_uniq_mm_ps) == "+"
                ]$score
            )
            covv_pl <- GRanges(covv_pl)
            covv_pl <- covv_pl[covv_pl$score > 0]
    ...
    if (
        !is.na(path_to_P_sites_uniq_plus_bw) |
            !is.na(path_to_P_sites_uniq_minus_bw)
    ) {
        for_ORFquant$P_sites_uniq <- input_P_sites_uniq
    }

    if (
        !is.na(path_to_P_sites_uniq_mm_plus_bw) |
            !is.na(path_to_P_sites_uniq_mm_minus_bw)
    ) {
        for_ORFquant$P_sites_uniq_mm <- input_P_sites_uniq_mm
    }

    cat(paste(
        "Calculating P-sites positions and junctions --- Done!",
        date(),
        "\n"
    ))
...
plot_ORFquant_results <- function(
    for_ORFquant_file,
    ORFquant_output_file,
    annotation_file,
    coverage_file_plus = NA,
    coverage_file_minus = NA,
    output_plots_path = NA,
    prefix = NA
) {
    cat(paste(
        "Plotting ORFquant results for ",
        ORFquant_output_file,
        " ... ",
        date(),
        "\n",
    ...
    if (is.na(prefix)) {
        prefix <- gsub(
            gsub(
                basename(ORFquant_output_file),
                pattern = "_final_ORFquant_results",
                replacement = ""
            ),
    ...
    if (is.na(output_plots_path)) {
        output_plots_path <- paste0(ORFquant_output_file, "_plots")
    }
    ...
    cat_biot[
        !cat_biot %in% c("protein_coding", "pseudogene")
    ] <- "non-coding RNA"
    tbid <- table(gids)
    tbid[tbid > 3] <- ">3"
    ...
    cat_biot[
        cat_biot != "protein_coding" & cat_biot_gene == "protein_coding"
    ] <- "non-coding isoform"
    cat_biot[grep(cat_biot, pattern = "pseudo")] <- "pseudogene"
    cat_biot[
        !cat_biot %in% c("protein_coding", "pseudogene", "non-coding isoform")
    ] <- "non-coding RNA"
    ...
    a <- a +
        theme(
            ...
            axis.text.y = element_text(angle = 45, vjust = 0.5, size = 15)
        )
    orfs_found_n <- a +
        theme(
            ...
            strip.background = element_rect(
                colour = "black",
                fill = c(rep("darkkhaki", 8), rep("red", 8))
            )
        )
    ...
    b <- ggplot(
        dfiso,
        aes(x = cat_biot, y = ORFs_tx.ORF_pct_P_sites, fill = cat_biot)
    )
    ...
    c <- ggplot(
        dfpnpm,
        aes(x = cat_biot, y = ORFs_tx.ORFs_pM + 1, fill = cat_biot)
    )
    ...
    c <- c +
        scale_y_log10(
            ...
            limits = c(1, max(dfpnpm$ORFs_tx.ORFs_pM) * 1.1),
    ...
    c <- c +
        theme(
            axis.title.y = element_text(size = 18),
            axis.text.y = element_text(angle = 45, vjust = 0.5, size = 16)
        )
    c <- c +
        theme(
    ...
    d <- d +
        scale_y_log10(
            ...
            limits = c(
                min(dfwid$width.ORFs_tx.),
                max(dfwid$width.ORFs_tx.) * 1.1
            )
        )
    #c<-c + geom_text(aes(x=cat_tx, y=ORFs_tx.ORFs_pM, hjust="top",label=ORFs_tx.ORFs_pM),colour="black",position = position_dodge(width = 1),size=5)
    d <- d + theme_bw()
    ...
    red_ex$gene_id <- GTF_annotation$trann$gene_id[match(
        names(red_ex),
        GTF_annotation$trann$transcript_id
    )]
    red_ex <- reduce(split(red_ex, red_ex$gene_id))

    ovv <- findOverlaps(red_ex, for_ORFquant$P_sites_all)
    aggo <- aggregate(
        for_ORFquant$P_sites_all$score[ovv@to],
        by = list(ovv@from),
        sum
    )
    cnts <- aggo[, 2]
    ...
    tb_bio <- cat_biot[match(names(tbid), gids)]
    max_ORF <- aggregate(
        ORFs_tx$ORF_pct_P_sites,
        by = list(ORFs_tx$gene_id),
        max
    )
    ...
    df$tb_bio <- factor(
        df$tb_bio,
        levels = c("protein_coding", "pseudogene", "non-coding RNA")
    )
    df <- table(df$Freq)
    df <- melt(df)
    ...
    a <- a +
        geom_text(
            aes(x = Var1, y = value, hjust = .5, vjust = -.8, label = value),
            check_overlap = TRUE,
            colour = "black",
            size = 4
        )
    ...
    a <- a +
        theme(
            axis.title.y = element_text(size = 20),
            axis.text.y = element_text(angle = 45, vjust = 0.5, size = 18)
        )
    orfsn_genes <- a +
        theme(
    ...
    c <- c +
        geom_text(
            aes(x = maxiso, y = value, hjust = .5, vjust = -.8, label = value),
            check_overlap = TRUE,
            colour = "black",
            size = 4
        )
    ...
    d <- d +
        scale_y_log10(
            breaks = c(1, 11, 101, 1001),
            limits = c(1, max(tpms_mult + 1) * 1.1),
            labels = c(0, 10, 100, 1000)
        )
    #b<-b + geom_text(aes(x=cat_tx, y=value, hjust="top",label=value),colour="black",position = position_dodge(width = 1),size=5)
    d <- d + theme_bw()
    ...
    ch_txs_sel <- CharacterList(split(
        selected_txs,
        GTF_annotation$trann$gene_id[match(
            selected_txs,
            GTF_annotation$trann$transcript_id
        )]
    ))
    ...
    a <- a +
        geom_text(
            aes(x = qnt, y = value, hjust = .5, vjust = -.8, label = value),
            check_overlap = TRUE,
            colour = "black",
            size = 4
        )
    ...
    gens_sel <- unique(GTF_annotation$trann$gene_id[
        GTF_annotation$trann$transcript_id %in% selected_txs
    ])
    tx_all <- GTF_annotation$trann$transcript_id[
    ...
    b$gene_id <- CharacterList(lapply(b$gene, function(x) {
        nms[x]
    }))
    nts_all <- c()
    nts_cds <- c()
    ...
    juns$tx_cds <- CharacterList(lapply(juns$tx_cds, function(x) {
        x[!is.na(x)]
    }))
    juns$tx_sel <- CharacterList(lapply(juns$tx_sel, function(x) {
        x[!is.na(x)]
    }))
    #juns_ok<-juns[elementNROWS(juns$tx_sel)>0 & elementNROWS(juns$tx_cds)>0]
    juns_ok <- juns[sapply(juns$gene_id, function(x) {
        sum(x %in% gens_sel) > 0
    })]
    ...
    if (!is.na(coverage_file_plus) & !is.na(coverage_file_minus)) {
        cat(paste(
            "Plotting alternative splice sites profiles ... ",
            date(),
            "\n",
            sep = ""
        ))

        aggregate_regions <- function(
            range,
            coverage_plus,
            coverage_minus,
            norm_x = NA,
        ...
        isovals <- cbind.data.frame(
            ORFs_tx$ORF_id_tr,
            ORFs_tx$ORF_pct_P_sites_pN
        )
        ORFs_gen$ORF_pct_P_sites_pN <- isovals[
            match(names(ORFs_gen), rownames(isovals)),
            2
        ]
        ORFs_gen$ORF_pct_P_sites_pN[which(is.na(
            ORFs_gen$ORF_pct_P_sites_pN
        ))] <- 0
        ORFs_gen$ORF_pct_P_sites_pN <- round(
            ORFs_gen$ORF_pct_P_sites_pN,
            digits = 4
        ...
        covisopl <- coverage(
            ORFs_gen[strand(ORFs_gen) == "+"],
            weight = ORFs_gen[strand(ORFs_gen) == "+"]$ORF_pct_P_sites_pN
        )
        covisomn <- coverage(
            ORFs_gen[strand(ORFs_gen) == "-"],
            weight = ORFs_gen[strand(ORFs_gen) == "-"]$ORF_pct_P_sites_pN
        ...
        ORFs_ok <- ORFs_spl_feat_maxORF[
            names(ORFs_spl_feat_maxORF) %in% ORFs_ok
        ]
        ...
        for (i in types) {
            ...
            if (i %in% c("up_3ss", "up_lastCDS")) {
                ...
                region$dist[as.vector(
                    strand(region) == "-"
                )] <- start(region[as.vector(strand(region) == "-")]) -
                    start(region[as.vector(strand(region) == "-")]$ref)
                region$dist <- as.numeric(region$dist)
                dst <- region$dist
                nms <- names(region)
                region1 <- promoters(
                    resize(region, width = 1, fix = "end"),
                    upstream = 25,
                    downstream = 0
                )
            ...
            if (
                i %in% c("down_3ss", "down_lastCDS", "same_3ss", "same_lastCDS")
            ) {
                region <- region[elementNROWS(region$ref) == 1]
                set.seed(666)
                if (length(region) > 1000) {
                ...
                region$dist[as.vector(strand(region) == "-")] <- start(
                    region[as.vector(strand(region) == "-")]$ref
                ) -
                    start(region[as.vector(strand(region) == "-")])
                region$dist <- as.numeric(region$dist)
                dst <- region$dist
                nms <- names(region)
                region1 <- promoters(
                    resize(unlist(region$ref), width = 1, fix = "end"),
                    upstream = 25,
                    downstream = 0
                )
                mcols(region1) <- mcols(region)
            ...
            if (i %in% c("down_5ss", "down_firstCDS")) {
                ...
                region$dist[as.vector(strand(region) == "-")] <- end(
                    region[as.vector(strand(region) == "-")]$ref
                ) -
                    end(region[as.vector(strand(region) == "-")])
                region$dist <- as.numeric(region$dist)
                #invert numbers for donwstr-upstr
                dst <- region$dist
                nms <- names(region)
                region2 <- promoters(
                    resize(region, width = 1, fix = "start"),
                    upstream = 25,
                    downstream = 0
                )
                mcols(region2) <- mcols(region)
            ...
            if (
                i %in% c("up_5ss", "up_firstCDS", "same_5ss", "same_firstCDS")
            ) {
                region <- region[elementNROWS(region$ref) == 1]
                set.seed(666)
                if (length(region) > 1000) {
                ...
                region$dist[as.vector(
                    ...
                    end(region[as.vector(strand(region) == "-")]$ref)
                region$dist <- as.numeric(region$dist)
                dst <- region$dist
                nms <- names(region)
                region2 <- promoters(
                    resize(unlist(region$ref), width = 1, fix = "start"),
                    upstream = 25,
                    downstream = 0
                )
                mcols(region2) <- mcols(region)
            ...
            if (!i %in% c("CDS_spanning")) {
                region1$iso <- 0
                region1$iso[as.vector(
                    strand(region1) == "+"
                )] <- mean(covisopl[region1[as.vector(strand(region1) == "+")]])
                region1$iso[as.vector(
                    strand(region1) == "-"
                )] <- mean(covisomn[region1[as.vector(strand(region1) == "-")]])
                region2$iso <- 0
                region2$iso[as.vector(
                    strand(region2) == "+"
                )] <- mean(covisopl[region2[as.vector(strand(region2) == "+")]])
                region2$iso[as.vector(
                    strand(region2) == "-"
                )] <- mean(covisomn[region2[as.vector(strand(region2) == "-")]])
                ...
                listrang[[i]] <- region
                listvals[[i]] <- aggregate_regions(
                    range = region,
                    coverage_plus = covpspl,
                    coverage_minus = covpsmn
                )
                listiso[[i]] <- aggregate_regions(
                    range = region,
                    coverage_plus = covisopl,
                    coverage_minus = covisomn
                )
            }
            ...
            if (i %in% c("CDS_spanning")) {
                ...
                start(ress[seq(1, length(region) * 2, by = 2)]) <- end(ress[seq(
                    1,
                    length(region) * 2,
                    by = 2
                )])
                ...
                region1_a <- promoters(
                    ress[seq(1, length(region) * 2, by = 2)],
                    upstream = 25,
                    downstream = 0
                )
                region2_a <- promoters(
                ...
                region2_b <- promoters(
                    ress[seq(2, length(region) * 2, by = 2)],
                    upstream = 25,
                    downstream = 0
                )
                region1_b <- promoters(
                ...
                region_a$dist <- sapply(region$ref, function(x) {
                    min(width(gaps(x)))
                })
                region_b$dist <- sapply(region$ref, function(x) {
                    min(width(gaps(x)))
                })
                ...
                region1_a$iso <- 0
                region1_a$iso[as.vector(
                    strand(region1_a) == "+"
                )] <- mean(covisopl[region1_a[as.vector(
                    strand(region1_a) == "+"
                )]])
                region1_a$iso[as.vector(
                    strand(region1_a) == "-"
                )] <- mean(covisomn[region1_a[as.vector(
                    strand(region1_a) == "-"
                )]])
                region1_b$iso <- 0
                region1_b$iso[as.vector(
                    strand(region1_b) == "+"
                )] <- mean(covisopl[region1_b[as.vector(
                    strand(region1_b) == "+"
                )]])
                region1_b$iso[as.vector(
                    strand(region1_b) == "-"
                )] <- mean(covisomn[region1_b[as.vector(
                    strand(region1_b) == "-"
                )]])
                region2_a$iso <- 0
                region2_a$iso[as.vector(
                    strand(region2_a) == "+"
                )] <- mean(covisopl[region2_a[as.vector(
                    strand(region2_a) == "+"
                )]])
                region2_a$iso[as.vector(
                    strand(region2_a) == "-"
                )] <- mean(covisomn[region2_a[as.vector(
                    strand(region2_a) == "-"
                )]])
                region2_b$iso <- 0
                region2_b$iso[as.vector(
                    strand(region2_b) == "+"
                )] <- mean(covisopl[region2_b[as.vector(
                    strand(region2_b) == "+"
                )]])
                region2_b$iso[as.vector(
                    strand(region2_b) == "-"
                )] <- mean(covisomn[region2_b[as.vector(
                    strand(region2_b) == "-"
                )]])
        ...
        same_all_5 <- promoters(
            resize(same_all, width = 1, fix = "center"),
            upstream = 50,
            downstream = 0
        )
        same_all_3 <- promoters(
            resize(same_all, width = 1, fix = "center"),
            upstream = 0,
            downstream = 50
        )
        ...
        listvals[["sameall_5"]] <- aggregate_regions(
            range = same_all_5,
            coverage_plus = covpspl,
            coverage_minus = covpsmn
        )
        listiso[["sameall_5"]] <- aggregate_regions(
            range = same_all_5,
            coverage_plus = covisopl,
            coverage_minus = covisomn
        )
        ...
        listvals[["sameall_3"]] <- aggregate_regions(
            range = same_all_3,
            coverage_plus = covpspl,
            coverage_minus = covpsmn
        )
        listiso[["sameall_3"]] <- aggregate_regions(
            range = same_all_3,
            coverage_plus = covisopl,
            coverage_minus = covisomn
        )
        ...
        for (i in seq_along(topl_ok)) {
            ...
            if (topl_ok[i] == "CDS_spanning") {
                ...
                names(rans_contr) <- paste(
                    names(rans_contr),
                    c(
                        rep("left", length(listrang[["same_3ss"]])),
                        rep("right", length(listrang[["same_3ss"]]))
                    ),
                    sep = "_"
                )
            }
            ...
            ok <- intersect(
                which(abs(rans$dist) > 25),
                which(rans$iso_1 < 101 & rans$iso_2 < 101)
            )
            ...
            if (
                topl_ok[i] %in%
                    c("up_3ss", "down_5ss", "down_firstCDS", "up_lastCDS")
            ) {
            ...
            if (topl_ok[i] == "CDS_spanning") {
                ...
                rans_contr$spanning[grep(
                    names(rans_contr),
                    pattern = "left"
                )] <- "upstream"
            }
            notcol <- names(mcols(rans))[
                !names(mcols(rans)) %in% names(mcols(rans_contr))
            ]
            rans$ref <- NULL
            ...
            cols <- colorRampPalette(c("dark blue", "blue", "red"))(8)[c(
                1,
                3,
                6,
                7
            ...
            means <- aggregate(
                dat$value,
                by = list(dat$group, dat$Position_from_splice_site, dat$type),
                mean
            )
            colnames(means) <- c(
                "group",
                "Position_from_splice_site",
                "type",
                "value"
            )
            ...
            if (topl_ok[i] == "CDS_spanning") {
                a <- a +
                    facet_grid(facets = . ~ spanning) +
                    xlab("Position from Splice Sites") +
                    ylab("Normalized coverage\nRibo-seq")
                b <- b +
                    facet_grid(facets = . ~ spanning) +
                    xlab("Position from Splice Sites") +
            ...
            if (topl_ok[i] %in% c("up_5ss")) {
                a <- a +
                    xlab("Position from Splice Site") +
                    ylab("Normalized coverage\nRibo-seq")
                b <- b +
                    xlab("Position from Splice Site") +
                    ylab("Normalized coverage\nORFquant ORFs")
            ...
            if (topl_ok[i] %in% c("up_firstCDS")) {
                a <- a +
                    xlab("Position from Start codon") +
                    ylab("Normalized coverage\nRibo-seq")
                b <- b +
                    xlab("Position from Start codon") +
                    ylab("Normalized coverage\nORFquant ORFs")
            ...
            if (
                !topl_ok[i] %in%
                    c("up_lastCDS", "up_firstCDS", "up_5ss", "CDS_spanning")
            ) {
                a <- a + xlab("") + ylab("")
                b <- b + xlab("") + ylab("")
            }
            ...
            if (topl_ok[i] == "CDS_spanning") {
                blocks = data.frame(
                    x1 = c(0, 0, 0.25, 0.39, 0.53, 0.67, .75),
                    x2 = c(1, .25, 0.34, 0.48, 0.62, 0.75, 1),
                    y1 = c(.5, .05, .225, .225, .225, .225, .05),
                    y2 = c(.85, .4, .235, .235, .235, .235, .4),
                    coll = c(
                        "red",
                        "blue",
                        "blue",
    ...
    for (i in names(list_ORFquant_plots)) {
        pdf(
            file = paste0(output_plots_path, "/", prefix, "_", i, ".pdf"),
            width = list_ORFquant_plots[[i]][["pars"]][1],
            height = list_ORFquant_plots[[i]][["pars"]][2]
        )
...
create_ORFquant_html_report <- function(
    input_files,
    input_sample_names,
    output_file
) {
    ...
    input_files <- paste(
        normalizePath(dirname(input_files)),
        basename(input_files),
        sep = "/"
    )
    output_file <- paste(
        normalizePath(dirname(output_file)),
    ...
    rmd_path <- paste(
        system.file(package = "ORFquant"),
        "/rmd/ORFquant_template.Rmd",
        sep = ""
    )
...
plot_orfquant_locus <- function(
    ...
    orfquantgr$feature[orfquantgr$feature == 'CDS'] <- names(orfquantgr)[
        orfquantgr$feature == 'CDS'
    ]

    #get correct col name
    if ('ORF_pM' %in% metacols) {
        quantcol = 'ORF_pM'
    } else {
        quantcol = 'TrP_pNpM'
    }
    ...
    plotTracks(
        list(itrack, maTrack, mdTrack, mgTrack),
        chromosome = chroms[i],
        add = TRUE
    )
    popViewport(1)
    ...
    #
    plotstart = start(selgenerange) -
        (0.2 * (end(selgenerange) - start(selgenerange)))
    plotend = end(selgenerange) +
        (0 * (end(selgenerange) - start(selgenerange)))
    # Gviz now imported via NAMESPACE
    legendwidth = 1 / 10
    ...
    plotTracks(
        ...
        add = TRUE,
        from = plotstart,
        to = plotend, #zoomed in on the orf in question
        sizes = c(1, 1, 1, 1, 1, 1, 1),
        rot.title = 0,
        cex.title = 1,
        title.width = 2.5,
        c(
