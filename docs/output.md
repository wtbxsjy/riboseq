...
- `orf_classification/`
  - `gencode_results.orfs.out` / `gencode_results.orfs.gtf` (mode: `gencode`)
  - `orfquant_classification.tsv` (mode: `orfquant`)
  - `orftype_classification.tsv` (mode: `orf_type`)

</details>

Classification output depends on the selected mode and always uses the unified ORF predictions as input.
