...
RiboCode detects translated ORFs from transcriptome-space alignments and is available when the pipeline has STAR or HISAT2 transcriptome BAMs. It is skipped automatically for BAM input mode.

<details markdown="1">
<summary>Output files</summary>

- `orf_predictions/ribocode/`
  - `*.txt`: Raw per-sample RiboCode ORF calls
  - `*_collapsed.txt`: Collapsed ORF calls used by QC aggregation
  - `*.gtf`: Genomic ORF annotations used by ORF unification and classification
  - `*.bed`: Genomic ORF intervals for browser-style inspection
  - `*.ribocode_failed.txt`: Failure marker written when RiboCode fails for a sample and placeholder outputs are emitted
...
The unified ORF predictions are generated by merging Ribo-TISH / Ribotricer / RiboCode / ORFquant postfilter outputs and de-duplicating ORFs by genomic coordinates.
