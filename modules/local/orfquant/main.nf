process ORFQUANT_RUN {
    ...
    # The container (orfquant_mirai.sif) has ORFquant built with:
    #   - GRangesList(lapply()) wrapper in select_quantify_ORFs() to prevent
    #     class degradation of ORFs_tx_position (root cause of ORFs_tx = 0)
    #   - .safe_field()/.safe_nested() that convert plain lists of GRanges
    #     instead of discarding them (defense in depth)
