...
31. **`extract_cds_fasta.py` 的终止密码子剥离必须要求长度已按密码子对齐**（verified 2026-09-11，PRJEB26593 重跑）。unify 元数据的 `sequence` 列是**原始**序列，带 1-2 nt 的边界噪声（`len(seq) % 3 != 0` 是常态）。旧逻辑 `if len(s) >= 3 and s[-3:] in STOP_CODONS` 对一条 37 nt / 12 aa 的 ORF 会把**最后 3 个字符**当密码子删掉，但那 3 个字符跨在密码子网格之外、根本不是密码子 —— 结果 NA 被截短 3 nt，`len(NA) != 3*len(AA)`。**这个 bug 被 PRICE contig 修复暴露**：修复前 PRICE ORF 的 `sequence` 全是 `N`，末尾是 `NNN`，永不匹配终止密码子表，所以 mismatch 恒为 0（假象）；修复后有真实序列，末尾凑出 TAA/TGA 就触发。PRJEB26593 新集上 26,373/112,984（23.3%）中招，其中 PRICE 来源占 38.9%、非 PRICE 占 0.0%。**修法**：`if len(s) % 3 == 0 and len(s) >= 3 and s[-3:] in STOP_CODONS`（截短逻辑保持不变，仍负责把 `len(s) > 3*len(p)` 的尾部噪声裁到 3*len(AA)）。修后 mismatch 归零，已应用于 `post_analysis/scripts/extract_cds_fasta.py` 及两个项目的副本。**排查提示**：任何时候看到 `len(NA) != 3*len(AA)`，先看该 ORF 的 `len(sequence) % 3` 是否为 0。
