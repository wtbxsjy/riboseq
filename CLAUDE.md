...
21. **Session lock on `-resume` after force-kill**: If Nextflow is killed abruptly (`kill -9`), the session cache at `.nextflow/cache/<uuid>/db/LOCK` persists and blocks the next `-resume`. **Fix**: `rm -rf .nextflow/cache/<session-uuid>/`.
