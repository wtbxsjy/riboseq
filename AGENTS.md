...
- Current branch: `dev`
- `HEAD` commit: `5215c93` (`Add progress logging to GENCODE classifier`)
- Working tree expectation after the recent cleanup: tracked files should be clean; `AGENTS.md` may be the only intended local change while this guidance is being updated/tracked.
- Recent housekeeping note: a previous batch of local modifications affecting many tracked files was verified to be pure `CRLF/LF` line-ending churn and was intentionally discarded.

## Recent Project Updates

Recent development has focused on ORF post-processing, workflow-generation tooling, and operational visibility:

1. **GENCODE classification progress logging** (`5215c93`)
   - `scripts/gencode-riboseqORFs/functions.py` now reports progress for long-running GENCODE classification stages (`orf_tags`, `exclude_variants`, `write_output`).
   - This improves observability for large jobs without changing classification semantics.

2. **Fast GENCODE classification path** (`b2e0a19`, merged via `d5ef8f1`)
   - New pipeline parameter: `--gencode_classify_impl` with `original | fast` (default `original`).
   - `scripts/classify_orfs_wrapper.py` can dispatch to `scripts/class_orf/run_gencode_classify_fast.py`.
   - The fast implementation parallelizes GENCODE classification by splitting ORFs into chromosome bins and running mapper subprocesses safely in parallel.
   - Benchmarking and comparison helpers were added under `scripts/class_orf/` to validate runtime gains and output consistency.

3. **Replicate BAM merging before ORF calling** (`fa88918`)
   - New pipeline parameter: `--merge_replicates` (default `false`).
   - In `workflows/riboseq/main.nf`, samples sharing the same samplesheet `group` can be merged after sORF BAM filtering and then passed through ORF-calling/QC tools alongside per-sample BAMs.
   - `scripts/prepare_workflow.py` was updated with `--group-map` and `--merge-replicates` support so generated run scripts can enable this path.

4. **ORF unification/classification workflow maturation** (`d03da6d`, `2ae2955`, `cf883e9`)
   - Per-tool ORF classification is supported when unification is skipped (`--skip_unify_orf_predictions true`).
   - ORFquant transcript-space classification was corrected and optimized to avoid earlier genomic-coordinate approximation errors and O(n^2) bottlenecks.
   - Documentation and `prepare_workflow.py` defaults/dry-run behavior were synced with the newer pipeline features.
